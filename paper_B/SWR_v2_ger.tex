...
Die SWR wurde im Rahmen einer gro\ss{}angelegten empirischen Anwendungsstudie \citep{Geiger2026Elite} entwickelt und validiert, die die kollektiven Weltbilder von 15~soziologischen Gruppen innerhalb der KI-Elite rekonstruiert. Die vorliegende Arbeit abstrahiert die Methode von diesem spezifischen Anwendungskontext und stellt sie als verallgemeinertes, transferierbares Rahmenwerk f\"ur alle Forscher vor, die kollektive Weltbilder rekonstruieren und vergleichen m\"ochten.
...
\emph{Zweitens} entwickelt die Arbeit einen sechsschichtigen Validierungsrahmen (V1--V6) f\"ur LLM-gest\"utzte qualitative Forschung, der Instrumentenkalibrierung (Run-Konvergenz), Prompt-Sensitivit\"atsanalyse, dimensionale Quantifizierung, Diskriminierbarkeitstest, Run-Konvergenz mit Varianzdekomposition und Adversarial Probing umfasst.
...
Die f\"unf Schritte werden in einer einzigen Konversationssitzung ausgef\"uhrt, sodass der vollst\"andige Kontext erhalten bleibt. F\"ur jede Syntheseeinheit wird eine \emph{neue} Sitzung gestartet, um Kontamination zwischen Einheiten zu verhindern. \emph{Instanztrennung} ist eine kritische methodische Anforderung: Wenn dieselbe Syntheseeinheit mehrfach f\"ur Reliabilit\"atssch\"atzungen (IMIIRR) verarbeitet wird, muss jeder Run in einer separaten Agenteninstanz ohne geteilten Konversationskontext stattfinden. Runs, die Kontext teilen, sind keine unabh\"angigen Messungen und k\"onnen nicht f\"ur Reliabilit\"atssch\"atzungen verwendet werden.
...
\subsection{V1: Instrumentenkalibrierung (Run-Konvergenz)}
\label{sec:kalibrierung}

Der prim\"are Reliabilit\"atstest f\"ur die SWR ist die \emph{Within-Model-Run-Konvergenz}: Dasselbe LLM verarbeitet dieselben Daten mehrfach, und die \"Ubereinstimmung \"uber Durchl\"aufe wird mittels Intraklassenkorrelationskoeffizienten (ICC) gemessen. Dieser Ansatz behandelt das LLM als \emph{einzelnes Messinstrument}, das kalibriert werden muss -- analog zur Etablierung von Test-Retest-Reliabilit\"at f\"ur eine psychometrische Skala. Akzeptanzkriterien:
\begin{itemize}[leftmargin=*]
  \item ICC $> .75$: Gute Kalibrierung; Instrument ist reliabel.
  \item $.60 < $ ICC $< .75$: Akzeptabel; Kernstrukturen stabil, Details variieren.
  \item ICC $< .60$: Ungen\"ugende Kalibrierung; Ergebnisse m\"oglicherweise run-abh\"angig.
\end{itemize}

\emph{Empirische Validierung (Pilotstudie, \citealp{Geiger2026Elite}).} Die Pilotstudie validiert Run-Konvergenz auf zwei Rigorosit\"atsstufen. Erstens wurden f\"unf Gruppen-Syntheseeinheiten jeweils zweimal unabh\"angig von Claude Opus~4.6 verarbeitet ($5 \times 2$-Design). Die resultierenden D01--D12-Profile zeigten exzellente Konvergenz: Pearson $r = .908$, ICC(2,1) $= .902$, MAE $= 0{,}40$, entsprechend einer Messpräzision von $\pm 0{,}2$ Skalenpunkten. Zweitens ergab eine erweiterte Validierung mit $N = 15$ vollst\"andig unabh\"angigen Einzelinstanz-Runs auf einer einzelnen Syntheseeinheit ICC(3,1) $= .847$ und mittlere SD $= 0{,}45$, mit perfekter \"Ubereinstimmung auf der stabilsten Dimension (D02). Jeder der 15~Runs wurde in einer separaten Agenteninstanz ohne geteilten Kontext ausgef\"uhrt -- was etabliert, dass die Reliabilit\"at der SWR eine genuine Eigenschaft der Methode ist (\emph{Intra-Model Inter-Instance Rating Reliability}, IMIIRR), kein Artefakt geteilten Konversationskontexts.

\emph{Instanztrennung} ist eine kritische methodische Anforderung: Jeder Analyserun muss eine separate Agenteninstanz ohne geteilten Kontext verwenden. Runs, die Kontext teilen (z.\,B.\ mehrere Analysen innerhalb einer einzigen Konversation), sind keine unabh\"angigen Messungen und k\"onnen nicht f\"ur Reliabilit\"atssch\"atzungen verwendet werden.

\emph{Inter-Modell-Vergleich} (Replikation mit verschiedenen LLMs) bleibt ein wichtiger zuk\"unftiger Validierungsschritt -- konzeptuell \"aquivalent zur Replikation durch andere Forscher mit anderen Instrumenten. Solche Vergleiche erfordern strikte Unabh\"angigkeitskriterien (separate Instanzen, kein geteilter Kontext) und sollten mindestens ein Open-Weight-Modell mit dokumentiertem, distinkt anderem Trainingskorpus einschlie\ss{}en.
...
\subsection{V5: Run-Konvergenz und Varianzdekomposition}
\label{sec:konvergenz}

F\"ur jede SE wird das Protokoll mindestens zweimal unabh\"angig durchgef\"uhrt (Temperatur~=~0 oder die niedrigste verf\"ugbare Einstellung; zu beachten ist, dass Determinismus selbst bei Temperatur~=~0 aufgrund von Hardware-Nichtdeterminismus in manchen Implementierungen nicht garantiert ist). Jeder Run muss in einer separaten Agenteninstanz ohne geteilten Kontext stattfinden (Instanztrennung). Die \emph{Run-Konvergenz-Rate} (Anteil der Dimensionswerte mit $\leq$1 Skalenpunkt Variation) soll $\geq$80\,\% betragen; die mittlere Standardabweichung soll unter 1{,}0 liegen.

\emph{Empirische Ergebnisse (Pilotstudie, \citealp{Geiger2026Elite}).} F\"unf Gruppen-Syntheseeinheiten wurden jeweils zweimal unabh\"angig von Claude Opus~4.6 verarbeitet, was eine Run-Konvergenz-Rate von 93\,\% ($\leq$1-Punkt-Variation) und eine MAE von 0{,}40 ergab. Eine erweiterte Validierung mit $N = 15$ unabh\"angigen Einzelinstanz-Runs best\"atigte diese Ergebnisse (ICC(3,1) $= .847$, mittlere SD $= 0{,}45$) und etablierte IMIIRR als robuste psychometrische Eigenschaft der SWR. Zus\"atzlich zeigte ein Expected-Discrepancy-Kontrollexperiment, dass eine synthetische konsistente Gruppe kein artifizielles Sagen-Handeln-Gef\"alle produzierte (MAE $= 0{,}50$), w\"ahrend das reale KI-Elite-Gef\"alle $2{,}5\times$ gr\"o\ss{}er war (MAE $= 1{,}25$) -- die Methode fabriziert also keine Diskrepanzen.
...
Eine SWR-Studie sollte Akzeptanzkriterien auf zwei Stufen erf\"ullen:

\emph{Stufe~1 (Instrumentenkalibrierung -- erforderlich):} (1)~Within-Model-Run-Konvergenz ICC~$> .75$ (oder $\geq 80\,\%$ der Dimensionswerte mit $\leq$1-Punkt-Variation); (2)~Prompt-Sensitivit\"ats-Varianzanteil~$< 15\,\%$ der Gesamtvarianz; (3)~mindestens ein externer Validierungsanker (Expert Judgment, Known-Groups-Validierung oder kreuzmodale Vorhersage gegen dokumentierte Handlungen).

\emph{Stufe~2 (Modellunabh\"angigkeit -- w\"unschenswert f\"ur Replikation):} (4)~Inter-Modell-Reliabilit\"at ICC~$> .70$ \"uber mindestens zwei Frontier-Modelle verschiedener Hersteller; (5)~Diskriminierbarkeitstest signifikant \"uber Zufallsbaseline.

Stufe-1-Kriterien sind Voraussetzung f\"ur Publikation; Stufe-2-Kriterien st\"arken die Generalisierbarkeit, sind aber konzeptuell \"aquivalent zur Replikation durch andere Forscher mit anderen Instrumenten. Die Pilotstudie \citep{Geiger2026Elite} erf\"ullt Stufe-1-Kriterien (1) und (3) (ICC(2,1)~$= .902$, erweitert auf ICC(3,1)~$= .847$ mit $N = 15$ unabh\"angigen Runs; kreuzmodale Vorhersage 72\,\% best\"atigt, 0\,\% widersprochen). Diese Kriterien sind als vorl\"aufige Richtwerte zu verstehen, die mit wachsender empirischer Erfahrung adjustiert werden k\"onnen.
...
\emph{Integrierte Qualit\"atssicherung.} Der sechsschichtige Validierungsrahmen (V1--V6) wurde in der Pilotanwendung mit 12~QA-Ma\ss{}nahmen empirisch getestet: Run-Konvergenz (ICC(2,1) $= .902$; erweitert auf ICC(3,1) $= .847$ mit $N = 15$ unabh\"angigen Runs, was IMIIRR etabliert), Expected-Discrepancy-Kontrolle (keine artifiziellen Gaps) und kreuzmodale Vorhersage (72\,\% best\"atigt, 0\,\% widersprochen) etablieren Reliabilit\"at und pr\"adiktive Validit\"at der Methode.
...
\emph{Vorgehen.} Das vollst\"andige Prompt-Protokoll (Sektion~\ref{sec:promptprotokoll}) wurde unter Verwendung von Claude (Anthropic) als einzigem LLM angewendet: Claude Sonnet~4.5 f\"ur batch-paralleles dimensionales Rating (9~Agenten) und Claude Opus~4.6 f\"ur Gruppen-Synthesen, qualitative Analysen und alle Validierungsexperimente. Blindung wurde f\"ur alle Syntheseeinheiten implementiert mittels eines dedizierten Skripts (\texttt{extract\_blinded.py}), das Personennamen, Firmennamen, Produktnamen und Universit\"atszugeh\"origkeiten durch neutrale Platzhalter ersetzt ([PERSON], [FIRMA], [PRODUKT], [UNIVERSIT\"AT]). Vier Operationalisierungen wurden ausgef\"uhrt: Op1 (Kernsynthesen: 41~gruppenbasierte; 100~individuelle als Zwischendatenschicht), Op2 (dimensionales Rating auf D01--D12), Op4 (kreuzmodale Vorhersage: Aussagen $\to$ vorhergesagte Handlungen $\to$ Vergleich mit realen Handlungen, f\"ur 5~Gruppen) und Op6 (9~strukturierte Vergleichsruns). Op3 (Textanalyse) und Op5 (Bottom-up-Clustering auf Embeddings) wurden nicht ausgef\"uhrt. Die prim\"are Analyseeinheit war durchgehend die soziologisch definierte Gruppe, nicht der individuelle Akteur.

\emph{Ergebnisse.} Die Companion-Studie \citep{Geiger2026Elite} berichtet die folgenden empirisch fundierten Befunde: Eine Hauptkomponentenanalyse der $100 \times 12$ Rating-Matrix identifiziert 5~Komponenten, die 80\,\% der Varianz erkl\"aren; HDBSCAN im vollst\"andigen 12-dimensionalen Raum klassifiziert 80--100\,\% der F\"alle als Rauschen, was best\"atigt, dass die vier Weltbild-Typen Webersche Idealtypen sind und keine emergenten Datencluster; Subspace-Clustering auf den zwei st\"arksten diskriminierenden Dimensionen (D07, D11) ergibt einen Silhouette-Score von 0{,}57; Kruskal-Wallis-Tests zeigen 6/12 Dimensionen mit $p < 0{,}001$ \"uber die vier Typen. Eine Gruppen-Sagen-Handeln-Dekomposition offenbart ein Simpson-Paradox: Das aggregierte Sagen-Handeln-Gef\"alle ist von Messrauschen nicht unterscheidbar, aber gruppenspezifische Gaps sind substanziell -- CEOs zeigen einen klassischen Erw\"unschtheits-Gap, Risiko-Warner zeigen prophetische Konsistenz (sie artikulieren bef\"urchtete Zuk\"unfte, gegen die sie aktiv arbeiten). Validierungsergebnisse: Run-Konvergenz mit ICC(2,1) $= .902$ ($5 \times 2$-Design), erweitert durch $N = 15$ vollst\"andig unabh\"angige Einzelinstanz-Runs mit ICC(3,1) $= .847$ und mittlerer SD $= 0{,}45$ (was IMIIRR als robuste psychometrische Eigenschaft etabliert), Expected-Discrepancy-Kontrolle (keine artifiziellen Gaps), kreuzmodale Vorhersage mit 72\,\% Best\"atigungsrate und 0\,\% Widerspruch \"uber 5~Gruppen (Op4), und ein Aggregations-Synthese-Vergleich mit moderater Konvergenz ($r = .623$). Die Companion-Studie dokumentiert insgesamt 12~QA-Ma\ss{}nahmen und erweitert den generischen V1--V6-Rahmen zu einem anwendungsspezifischen Qualit\"atsregime. Zwei Validierungsschichten bleiben ungetestet: V2 (Prompt-Sensitivit\"at) und V4 (Diskriminierbarkeitstest). Inter-Modell-Vergleich (mit strikten Instanztrennung-Kriterien) bleibt Priorit\"at f\"ur k\"unftige Arbeiten.
...
\emph{Validierungsabdeckung.} Von den sechs in Sektion~\ref{sec:qualitaet} vorgeschlagenen Validierungsschichten liefert die Pilotanwendung Evidenz f\"ur vier: V1 (Instrumentenkalibrierung: Run-Konvergenz mit ICC(2,1) $= .902$ f\"ur das $5 \times 2$-Design, erweitert auf ICC(3,1) $= .847$ f\"ur $N = 15$ unabh\"angige Einzelinstanz-Runs), V3 (Dimensionsstruktur via PCA: 5~Komponenten, 80\,\% Varianz), V5 (Run-Konvergenz: MAE $= 0{,}40$, mittlere SD $= 0{,}45$ f\"ur $N = 15$, plus Expected-Discrepancy-Kontrolle) und partielle Evidenz f\"ur Op4 (kreuzmodale Vorhersage: 72\,\% Best\"atigungsrate, 0\,\% Widerspruch \"uber 5~Gruppen). V2 (Prompt-Sensitivit\"at), V4 (Diskriminierbarkeitstest) und V6 (Adversarial Probing) verbleiben als offene Validierungsaufgaben. Die abgeschlossenen Validierungen etablieren, dass die SWR \emph{reliable} (V1/V5, IMIIRR), \emph{nicht-artifizielle} (Diskrepanzkontrolle) und \emph{pr\"adiktiv valide} (Op4) Gruppen-Weltbild-Rekonstruktionen produziert. H\"ochste Priorit\"at hat V2 (Prompt-Sensitivit\"atsanalyse); Inter-Modell-Vergleich (mit strikter Instanztrennung) ist die zweite Priorit\"at.
...
\emph{Zirkularit\"atsrisiko im Validierungsrahmen.} Eine potenzielle Zirkularit\"at besteht: Dasselbe LLM, das die Weltbild-Rekonstruktion generiert, wird (teilweise) auch zur Qualit\"atsbewertung dieser Rekonstruktion eingesetzt (z.\,B. durch kreuzmodale Vorhersage, Op4, und den Identifikationstest, V4). Dies wirft die Frage auf, ob die Validierung die interne Konsistenz des LLM best\"atigt statt seiner Korrespondenz mit der empirischen Realit\"at. Die Pilotstudie mildert dieses Risiko durch drei Ma\ss{}nahmen: (a)~Run-Konvergenztests (V1/V5) etablieren, dass das Instrument stabil ist (ICC(2,1) $= .902$, ICC(3,1) $= .847$); (b)~die Expected-Discrepancy-Kontrolle zeigt, dass das LLM keine Sagen-Handeln-Gaps fabriziert; (c)~kreuzmodale Vorhersage (Op4) vergleicht LLM-generierte Vorhersagen mit \emph{empirischer Ground Truth} (real dokumentierten Handlungen). Dennoch w\"urde der Validierungsrahmen substanziell gest\"arkt durch die Einbeziehung externer, nicht-LLM-basierter Validierung --- beispielsweise Inter-Modell-Replikation, Member Checking durch die analysierten Gruppen oder konvergente Validierung gegen unabh\"angig kodierte qualitative Analysen.
...
\emph{SWR und LLM-as-Participant.} Im Paradigma von \citet{Argyle2023} \emph{simuliert} das LLM eine Person; in der SWR \emph{analysiert} es die \"Uberzeugungen einer Gruppe (oder, als Spezialfall, eines Individuums). Die SWR operiert prim\"ar auf der Ebene soziologisch definierter Gruppen und rekonstruiert kollektive Weltbilder aus aggregierten Daten, statt individuelle demographische Befragte zu simulieren.
...
\emph{Drittens} die Integration eines vollst\"andigen Validierungsrahmens und einer formalisierten Variablenarchitektur. Die sechs Validierungsstrategien (V1--V6, in der Pilotstudie zu 12~QA-Ma\ss{}nahmen erweitert) adressieren die spezifischen Herausforderungen LLM-gest\"utzter Forschung; die vierstufige Variablenarchitektur (AVs, Ops, MVs, UVs) und neun Reduktionskriterien bieten einen rigorosen Designrahmen f\"ur SWR-Studien.

\emph{Viertens} empirische Validierung. Die Pilotstudie demonstriert, dass die SWR reliable (ICC(2,1) $= .902$; ICC(3,1) $= .847$ mit $N = 15$ unabh\"angigen Runs), nicht-artifizielle (Expected-Discrepancy-Kontrolle) und pr\"adiktiv valide (72\,\% kreuzmodale Best\"atigung, 0\,\% Widerspruch) Gruppen-Weltbild-Rekonstruktionen produziert. Die $N = 15$-Validierung etabliert IMIIRR (Intra-Model Inter-Instance Rating Reliability) als robuste psychometrische Eigenschaft.
...
\noindent Die Pilotstudie der SWR -- einschlie\ss{}lich aller Datenerhebungsskripte, Analysecode und Gruppen-Synthese-Outputs -- ist \"offentlich verf\"ugbar unter: \url{https://github.com/research-line/ai-elite-swr}. Die Companion-Anwendungsstudie ist verf\"ugbar unter: \url{https://doi.org/10.5281/zenodo.18736737}. Das Repository umfasst: (a)~Prompt-Templates f\"ur Kernsynthese (Op1), dimensionales Rating (Op2), kreuzmodale Vorhersage (Op4) und strukturierte Vergleiche (Op6); (b)~die Dimensionsdefinitionen (D01--D12) mit Rating-Ankern; (c)~die Implementierung der Reduktionskriterien; (d)~alle Blindungs- und Datengenerierungsskripte; (e)~die $100 \times 12$ Rating-Matrix und alle Gruppen-Synthese-Outputs; sowie (f)~Validierungsanalyse-Skripte (Run-Konvergenz, Expected-Discrepancy-Kontrolle, Aggregationsvergleich, kreuzmodale Vorhersage). Forschende, die die SWR f\"ur ihre eigenen Populationen replizieren oder adaptieren m\"ochten, sind eingeladen, diese Materialien als Ausgangspunkt zu nutzen.
