...
\emph{Empirical validation (pilot study, \citealp{Geiger2026Elite}).} Five group-level synthesis units were each processed twice independently by Claude Opus~4.6. The resulting D01--D12 profiles showed excellent convergence: overall Pearson $r = .908$, ICC(2,1) $= .902$, MAE $= 0.40$ on the 10-point scale, corresponding to a measurement precision of $\pm 0.2$ scale points. All 12~dimensions were stable across runs (mean $|\Delta| \leq 0.8$). This establishes that the SWR method, when applied with a single calibrated instrument, produces reliable group-level worldview reconstructions.
...
Each worldview reconstruction is projected onto twelve dimensions (D01--D12) equally covering the three DVs (four dimensions each, 10-point scale). Dimensions are to be defined application-specifically and should fulfill the following validity criteria:
...
An SWR study is considered methodologically acceptable when the following criteria are met: (1)~inter-LLM reliability ICC~$> 0.70$; (2)~prompt sensitivity variance share~$< 15\%$ of total variance; (3)~run convergence rate~$\geq 80\%$; (4)~identification test significantly above chance baseline; (5)~at least one external validation criterion (expert judgment or known-groups validation). These criteria should be understood as preliminary benchmarks that may be adjusted as empirical experience grows.
...
\emph{First}, the identification of a methodological gap and a structural solution. No existing method can systematically reconstruct the \emph{integrated belief system} of a group as a coherent whole and then compare it with another group's belief system. SWR fills this gap by exploiting the synthetic integration capability of LLMs---their ability to compose heterogeneous fragments into coherent wholes that never existed as explicit objects.
...
\noindent A complete application of the SWR method, including all data collection scripts, qualitative coding scripts, analysis code, and the resulting dataset of 100 AI leader worldview profiles, is publicly available at: \url{https://github.com/lukisch/ai-elite-worldview}. The companion application paper is available at: \url{https://doi.org/10.5281/zenodo.18736737}. The repository includes: (a)~the complete prompt templates for all five protocol steps and all six operationalizations; (b)~the dimensional definitions (D01--D12) with rating anchors; (c)~the reduction criteria implementation; (d)~all blinding scripts; and (e)~reproducibility scripts for inter-LLM comparison. Researchers wishing to replicate or adapt SWR for their own populations are encouraged to use these materials as a starting point.
...
\begin{tabularx}{\textwidth}{l l X}
  ...
  D07 & Power concentration & 1 = distribute \ldots{} 10 = centralize \\
...
\begin{tabularx}{\textwidth}{l l X}
  ...
  \textbf{Code} & \textbf{Name} & \textbf{Poles} \\
  \midrule
  D09 & Philanthropy & 1 = replaceable \ldots{} 10 = unique \\
...
\bibitem[Geiger(2026b)]{Geiger2026b}
  ...
  \textit{Preprint}. Independent Researcher, Bernau, Germany.
