...
Application to individual actors is possible as a special case but is methodologically more demanding: blinding becomes critical because the LLM's training data may contain prior knowledge about specific individuals, whereas for groups---whose collective worldview has never been explicitly articulated anywhere---the contamination risk is structurally reduced. SWR was developed and empirically hardened in a large-scale application study \citep{Geiger2026b} that reconstructs the collective worldviews of 15~sociological groups within the AI elite. The present paper abstracts the method from that specific application context and presents it as a generalized, transferable framework for any researcher seeking to reconstruct and compare collective worldviews.
...
The five steps are executed in a single conversation session so the complete context is preserved. For each synthesis unit, a \emph{new} session is started to prevent contamination between units. \emph{Instance separation} is a critical methodological requirement: when the same synthesis unit is processed multiple times for reliability estimation (IMIIRR), each run must use a separate agent instance with no shared conversational context. Runs that share context are not independent measurements and cannot be used for reliability assessment.
...
\emph{Empirical validation (pilot study, \citealp{Geiger2026b}).} The pilot study validates run convergence at two levels of rigor. First, five group-level synthesis units were each processed twice independently by Claude Opus~4.6 ($5 \times 2$ design). The resulting D01--D12 profiles showed excellent convergence: overall Pearson $r = .908$, ICC(2,1) $= .902$, MAE $= 0.40$ on the 10-point scale, corresponding to a measurement precision of $\pm 0.2$ scale points. All 12~dimensions were stable across runs (mean $|\Delta| \leq 0.8$). Second, an extended validation with $N = 15$ fully independent single-instance runs on a single synthesis unit yielded ICC(3,1) $= .847$ and Mean SD $= 0.45$, with perfect agreement on the most stable dimension (D02). Each of the 15~runs was executed in a separate agent instance with no shared context---establishing that SWR's reliability is a genuine property of the method (\emph{Intra-Model Inter-Instance Rating Reliability}, IMIIRR), not an artifact of shared conversational context. This establishes that the SWR method, when applied with a single calibrated instrument, produces reliable group-level worldview reconstructions.

\emph{Instance separation} is a critical methodological requirement: each analysis run must use a separate agent instance with no shared context. Runs that share context (e.g., multiple analyses within a single conversation) are not independent measurements and cannot be used for reliability estimation.

\emph{Inter-model comparison} (replication with different LLMs) remains an important future validation step---conceptually equivalent to replication by other researchers using different instruments. Current frontier LLMs share substantial overlap in training corpora and undergo similar alignment procedures (RLHF/RLAIF). Such comparison requires strict independence criteria (separate instances, no shared context) and should include at least one open-weight model with a documented, distinct training corpus.
...
\emph{Empirical results (pilot study, \citealp{Geiger2026b}).} Five group-level synthesis units were each processed twice independently by Claude Opus~4.6, yielding a run convergence rate of 93\% ($\leq$1 point variation) and an overall MAE of 0.40---well within the acceptance thresholds. The best convergence was observed for ideologically cohesive groups (ICC $= .969$ for regulation critics), the weakest for groups with greater internal tension (ICC $= .826$ for risk warners). An extended validation with $N = 15$ independent single-instance runs confirmed these results (ICC(3,1) $= .847$, Mean SD $= 0.45$), establishing IMIIRR as a robust psychometric property of SWR. Additionally, an expected-discrepancy control experiment showed that a synthetic consistent group produced no artifactual say-do gap (MAE $= 0.50$), while the real AI elite gap was $2.5\times$ larger (MAE $= 1.25$), confirming that the method does not fabricate discrepancies.
...
\emph{Integrated quality assurance.} The validation framework (V1--V6) has been empirically tested in the pilot application with 12~QA measures: run convergence (ICC(2,1) $= .902$; extended to ICC(3,1) $= .847$ with $N = 15$ independent runs, establishing IMIIRR), expected-discrepancy control (no artifactual gaps), and cross-modal prediction (72\% confirmed, 0\% contradicted) establish the method's reliability and predictive validity.
...
\emph{Results.} The companion study \citep{Geiger2026b} reports the following empirically grounded findings: a principal component analysis of the $100 \times 12$ rating matrix identifies 5~components explaining 80\% of variance; HDBSCAN on the full 12-dimensional space classifies 80--100\% of cases as noise, confirming that the four worldview types are Weberian ideal types rather than emergent data clusters; subspace clustering on the two strongest discriminating dimensions (D07, D11) yields a silhouette score of 0.57; Kruskal-Wallis tests show 6/12 dimensions with $p < 0.001$ across the four types. A group-level say-do decomposition reveals a Simpson's paradox: the aggregate say-do gap is indistinguishable from measurement noise, but group-specific gaps are substantial---CEOs show a classical desirability gap, risk warners exhibit prophetic consistency (articulating feared futures they actively work against). Validation results include: run convergence with ICC(2,1) $= .902$ ($5 \times 2$ design) extended by $N = 15$ fully independent single-instance runs with ICC(3,1) $= .847$ and Mean SD $= 0.45$ (establishing IMIIRR as a robust psychometric property), an expected-discrepancy control confirming no artifactual say-do gap, cross-modal prediction with 72\% confirmation rate and 0\% contradiction across 5~groups (Op4), and an aggregation-synthesis comparison showing moderate convergence ($r = .623$) with a systematic amplification effect (71.7\% of group-dimension combinations amplified by direct synthesis). The companion study documents 12~QA measures in total, expanding the generic V1--V6 framework into an application-specific quality regime. Two validation layers remain untested: V2 (prompt sensitivity) and V4 (discriminability test). Inter-model comparison (testing model-independence with strict instance separation criteria) remains a priority for future work.
...
\emph{Validation coverage.} Of the six validation layers proposed in Section~\ref{sec:quality}, the pilot application provides evidence for four: V1 (instrument calibration: run convergence with ICC(2,1) $= .902$ for $5 \times 2$ design, extended to ICC(3,1) $= .847$ for $N = 15$ independent single-instance runs), V3 (dimensional structure via PCA: 5~components, 80\% variance), V5 (run convergence: MAE $= 0.40$, Mean SD $= 0.45$ for $N = 15$, plus expected-discrepancy control confirming no artifactual say-do gaps), and partial evidence for Op4 (cross-modal prediction: 72\% of statement-based behavioral predictions confirmed by real actions across 5~groups, 0\% contradicted). V2 (prompt sensitivity), V4 (discriminability test), and V6 (adversarial probing) remain as open validation tasks. The completed validations establish that SWR produces \emph{reliable} (V1/V5, IMIIRR), \emph{non-artifactual} (discrepancy control), and \emph{predictively valid} (Op4) group-level worldview reconstructions. The highest-priority open validation task is V2 (prompt sensitivity analysis): since SWR is fundamentally a prompt-based method, demonstrating that results are robust to prompt paraphrasing is essential for methodological credibility. Inter-model comparison (testing model-independence with strict instance separation) is the second priority.
...
\emph{SWR and LLM-as-Participant.} In the paradigm of \citet{Argyle2023}, the LLM \emph{simulates} a person; in SWR, it \emph{analyzes} the beliefs of a group (or, as a special case, an individual). Moreover, SWR operates primarily at the level of sociologically defined groups, reconstructing collective worldviews from aggregated data rather than simulating individual demographic respondents.
...
\emph{Fourth}, empirical validation. The pilot application demonstrates that SWR produces reliable (ICC(2,1) $= .902$; ICC(3,1) $= .847$ with $N = 15$ independent runs), non-artifactual (expected-discrepancy control), and predictively valid (72\% cross-modal confirmation, 0\% contradicted) group-level worldview reconstructions. The $N = 15$ validation establishes IMIIRR (Intra-Model Inter-Instance Rating Reliability) as a robust psychometric property. The validation framework (V1--V6, expanded to 12~QA measures in the pilot) provides a structured quality regime for future SWR studies.
