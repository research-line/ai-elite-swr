%% HINWEIS: Die Kombi-PDF wird per Merge aus EN+GER erzeugt.
%% Master: EN-Version. Bei Aenderungen EN zuerst, dann DE synchronisieren.
%% Stand: 2026-06-12 v8.1 -- Maintenance, Data Availability, DE-EN sync
%% =============================================================================
%% KI_Elite_v3_en.tex -- English version
...
\begin{abstract}
\noindent
This study reconstructs the collective worldviews of sociologically defined groups within the 100 most influential AI actors between 2010 and 2026. To address the research questions, the study develops synthetic worldview reconstruction (SWR) -- an LLM-assisted synthesis method that reconstructs coherent belief systems from group-aggregated data, here operationalized along 12~worldview dimensions. The reconstructed collective profile suggests a \emph{technological messianism with ambivalence structure}: a high sense of mission and techno-determinism combined with low anthropological valuation and egalitarianism. Longitudinally, the data indicate a transition from utopian techno-optimism to tragic acceleration compulsion. A group-level say-do decomposition indicates a Simpson's-paradox pattern: the aggregate gap is indistinguishable from measurement noise, but group-specific gaps are substantial -- CEOs exhibit a classical desirability gap (rhetoric more egalitarian than action), while risk warners show prophetic consistency (articulating feared futures they actively work to prevent). An influence-strata analysis suggests that the most powerful actors are assigned the most intense worldviews, with human appreciation decreasing as influence rank rises. Four worldview types are identified -- Architect, Guardian, Innovator, Liberator -- whose inter-group comparisons point to a power-moderation paradox. The strongest predictors of worldview difference are ideological stance, institutional role, and gender. The primary contribution lies in nine systematic inter-group analyses indicating how structural position may shape collective belief within the AI elite. All findings rest on LLM-generated reconstructions and should be understood as empirically supported hypotheses.
...
What is still \emph{missing} in this research field is a systematic, data-driven analysis of the AI elite as a \emph{structured collective} -- one that distinguishes between sociologically meaningful subgroups and maps their respective belief systems. To the author's knowledge, no existing study examines the following questions in an empirically systematic way: How do the collective worldviews of different groups within the AI elite (CEOs vs.\ academics, accelerationists vs.\ risk warners, men vs.\ women) relate to each other? Do they form coherent types? How have these group-level worldviews changed over the period 2010 to 2026? And do public beliefs align with actual behavior at the group level? Empirical elite research is also relevant here: \citeauthor{Mills1956}'s analysis of the ``power elite'' as interlocking networks of business, military, and political power provides a structural model that appears transferable to the AI elite, with technological competence replacing military power as the central resource.

Beyond this empirical gap, there is a \emph{methodological} gap. Few established methods support the systematic reconstruction of worldviews from public statements at scale. Classical qualitative methods do not scale easily; standardized surveys require access to respondents; quantitative text-mining approaches often capture surface patterns rather than latent belief structures. The present study responds to this dual gap with a new approach -- synthetic worldview reconstruction (SWR) -- whose methodology is presented in full in Section~\ref{sec:methodik}.
...
The study makes a threefold contribution. \emph{Methodologically:} To address the research questions, the study develops the synthesis pipeline of synthetic worldview reconstruction (SWR; see Section~\ref{sec:methodik}) -- an LLM-assisted method for synthesizing group-aggregated data into coherent collective belief representations -- and applies dimensional rating as one specific operationalization of this pipeline to the empirical material of this study. \emph{Empirically:} It provides a systematic worldview analysis of the AI elite as a structured collective, based on 3,132 data points spanning 16 years and differentiated across 9~systematic inter-group comparisons (by role, stance, gender, company, epoch). \emph{Societally:} It offers a basis for informed public deliberation by identifying the collective ideological patterns -- trends, group differences, say-do gaps -- that shape AI governance and design decisions.
...
The use of large language models as instruments for social-scientific research is a rapidly expanding field. \citet{Tai2024} examined LLMs for deductive coding; \citet{Hayes2025} showed that LLMs enable a conversational approach to qualitative data; \citet{NicmanisSpurrier2025} provide a practice-oriented introduction. \citet{OrnsteinEtAl2025} show that LLMs can perform multidimensional ideological scaling with high reliability -- direct methodological support for the SWR approach. Work on synthetic personas \citep{Ge2024} has demonstrated the scalability of this broader approach but pursued a \emph{different} goal: it uses synthetic personas for \emph{data generation}, whereas the present study uses them for \emph{reconstruction} of coherent worldviews from \emph{real} data. The synthetic persona in our procedure is not a simulacrum but an analytical condensation instrument -- closer to the Weberian ideal type than to the simulation agent \citep[cf.][]{Booth1961, NgRussell2000, JaraEttinger2019}. A scoping review of 81 GenAI-persona studies found that 45\,\% reported no evaluation and 86\,\% relied on a single GPT model, with recurring risks of closed-loop generation and evaluation \citep{AminEtAl2026GenAIPersonas}. For SWR, this literature is an adjacent methodological guardrail rather than a direct validation study: reconstructing group-level ideal types from documented evidence differs from generating synthetic respondents, but that distinction alone neither establishes external validity nor removes dependence on one model family.
...
This section presents the implemented methodology. Synthetic worldview reconstruction (SWR) is an LLM-assisted method for inferring collective belief systems from publicly observable data. Its working premise is that worldview analysis can be formulated as an inverse problem: given the observable outputs (statements and actions) of a sociologically defined group, reconstruct the latent belief system that would most coherently generate these outputs. The method uses the capacity of large language models to synthesize heterogeneous textual evidence into coherent representations -- not by retrieving stored knowledge about individuals, but by constructing a collective portrait from the presented data. This section is self-contained for evaluation and replication.
...
SWR defines worldview analysis as an inverse problem: from the aggregated observable statements and actions of a \emph{group's} members, it infers the collective belief system $W_G^*$ that would most coherently generate these outputs. Formally: given the set of all public statements $\{A_1, \ldots, A_n\}$ and documented actions $\{H_1, \ldots, H_m\}$ of a sociologically defined group $G$, reconstruct $W_G^*$ such that the totality of observed outputs is maximally coherent under $W_G^*$. The solution consists of constructing a \emph{fictive synthetic person} -- a Weberian ideal type that represents the group's worldview in ideal-typical form, reducing individual biographical noise and PR strategies.

SWR uses a structural advantage of group-level analysis: no LLM training corpus is likely to contain ``the collective worldview of CEOs'' or ``the shared belief system of risk warners'' as a pre-formed object. The collective worldview is \emph{created} by the synthesis, not retrieved from memory. This may reduce the contamination risk that is the primary methodological concern for LLM-based reconstruction -- an advantage that does not hold to the same degree for individual-level analysis, where the LLM may draw on extensive stored knowledge about specific public figures.
...
\begin{table}[htbp]
  ...
  \label{tab:ops}
  \small
  \begin{tabularx}{\textwidth}{l L{3.1cm} Y l}
    \toprule
...
\subsubsection{Mapping: Research Questions, Operationalizations, and Core SUs}

Table~\ref{tab:zuordnung} gives a compact overview of which operationalizations and synthesis units are assigned to the six research questions.

\begin{table}[htbp]
  \centering
  \caption{Mapping of research questions, operationalizations, and core SUs.}
  \label{tab:zuordnung}
  \footnotesize
  \begin{tabularx}{\textwidth}{l l l Y}
    \toprule
    \textbf{RQ} & \textbf{Ops} & \textbf{Core SUs} & \textbf{Object} \\
    \midrule
    RQ1 & Op1, Op2, Op3 & 1, 4, 5 & Collective and Top-10 profile, core beliefs \\
    RQ2 & Op2, Op3 & 6--26 & Yearly and epoch syntheses, trends \\
    RQ3 & Op2, Op4 & 2, 3, 42--43 & Statements vs.\ actions, congruence test \\
    RQ4 & Op2, Op5 & 27--41 & Group syntheses, cluster identification \\
    RQ5 & Op2, Op6 & 27--41 & Subgroup comparisons \\
    RQ6 & Op2 & 4, 27--30 & Power-worldview relationship \\
    \bottomrule
  \end{tabularx}
\end{table}
...
(k)~\textbf{Influence strata comparison:} Three rank-based strata (Top~10: ranks 1--10, avg 7.7; Middle: ranks 40--60, avg 6.4; Bottom~10: ranks 91--100, avg 6.1) were independently synthesized and rated. The comparison indicates an influence gradient: the most powerful actors are assigned the most intense worldviews. Two dimensions showed an \emph{inverse} gradient: D09~(human appreciation: Top~4, Middle~6, Bottom~8) and D10~(posthumanism: Top~8, Middle~5, Bottom~3) -- the most powerful actors are reconstructed as the most posthumanist and least human-appreciating.
...
The ten most powerful AI actors are reconstructed as holding the same worldview in a more intense form (Avg~7.7). On ten of twelve dimensions, Top~10 values lie exactly one scale point above the collective. The two exceptions -- human appreciation (D09=4) and egalitarianism (D11=4) -- are \emph{lower}. The more powerful the position, the more pronounced the sense of mission and the more detached the view of humanity.
...
The higher the worldview intensity, the lower the values for human appreciation and egalitarianism, and the higher the acceptance of power concentration. The investor profile marks the extreme case: maximal self-efficacy (D02=10) with minimal sense of responsibility (D04=4) -- ``power without accountability.'' The Top~10 lie exactly one point above the collective on ten of twelve dimensions -- with one notable exception: human appreciation and egalitarianism lie \emph{below}. In the reconstructed profiles, power is associated with greater intensity on almost all dimensions except those that question power itself.

\medskip
\noindent\textbf{Core finding RQ6:} The reconstructed profiles suggest an association between economic power and worldview extremity, combined with an inverse association with egalitarianism and responsibility. Since both sides of this observation rest on LLM reconstructions, it remains unclear whether the association reflects empirical reality or internal model coherence. If robust, this would be the finding most relevant for democratic theory.
...
The results \emph{suggest} that the AI elite, analyzed at the group level, exhibits a pattern that can be described in ideal-typical form as \emph{technological messianism with structural ambivalence}. The metaphor of ``messianism'' is used analytically here -- as a label for the combination of high sense of mission, techno-determinism, and urgency -- not as a value judgment or psychologizing attribution. The term also differs from related concepts: \citeauthor{Morozov2013}'s ``solutionism'' captures a problem-solving stance rather than quasi-religious urgency; ``techno-utopianism'' implies positive future belief, which RQ2 suggests is precisely eroding; ``technological messianism'' refers more specifically to the persistence of a redemptive imperative \emph{despite} waning future optimism. The six research questions coalesce into a coherent picture: RQ1~points to the ``dual trough'' at D09 and D11 as a collective feature; RQ2~suggests an erosion of the utopia complex across epochs; RQ3~indicates directional say-do discrepancies on specific dimensions (D07, D11), though the overall gap magnitude requires cautious interpretation; RQ4~differentiates four worldview types with antagonistic collective positions; RQ5 and RQ6 complete the picture through the power-moderation paradox. The inter-group comparisons (RQ5) constitute the analytical heart of the study: the finding that institutional role, ideological stance, and gender systematically shape collective belief structures carries direct implications for AI governance and democratic theory. All findings are based on LLM-generated group-level reconstructions and should therefore be understood as \emph{empirically supported hypotheses}, not as proven facts. Formulations such as ``shows'' and ``reveals'' are throughout to be read as ``the data suggest.''

Four features are especially notable: first, the directional consistency of the say-do discrepancies across measurement conditions (D07/D11 stable in v1 and v2); second, the retreat of posthumanism (D10 from 10 to 6 -- contradicting the common assumption of an increasingly posthumanist orientation); third, the existence of the Liberator type (opposition to power concentration also from the libertarian side); and fourth, the consistency of group-level patterns across multiple independent syntheses.


\subsection{Situating the Findings in the Literature}
\label{sec:diskussion:einordnung}

\citeauthor{Morozov2013}'s solutionism thesis is strongly reflected in the reconstructed profiles (D05=8--9, D06=7--9). However, solutionism is not a monolithic block: risk warners (D05=4) share it substantially less. It appears as the ideology of the most powerful segment, not of the AI elite as such.

\citeauthor{Daub2020}'s analysis of philosophical deep structures is echoed by the narrative syntheses. What the present study adds is \emph{quantification} and the demonstration that intensity systematically increases with power position within the reconstructed profiles.

The Californian Ideology diagnosed by \citet{BarbCam1996} finds a contemporary counterpart in the Liberator type -- reconstructed here almost thirty years later.

\citeauthor{Coeckelbergh2020}'s assumption of increasing enhancement orientation is partly called into question by the reconstructed decline of posthumanism (D10 from 10 to 6). At least three alternative explanations must be considered: (a)~the decline could be a \emph{discourse artifact} -- posthumanist themes became less salient in public communication after 2020, reducing the density of corresponding statements; (b)~the position may not have disappeared but been translated into new terms (``AGI'' rather than ``enhancement''); or (c)~the trend could reflect strategic \emph{self-censorship}, as explicitly posthumanist rhetoric has become politically more toxic.
...
The four-cluster solution can furthermore be read through field theory: the antagonistic structure between Architect (economic capital) and Guardian (symbolic capital) reproduces the pattern \citet{Bourdieu1992} diagnosed for the intellectual and artistic field -- an \emph{inverted economic world}. In the AI field, those who control the most resources have the least legitimation through public-interest orientation -- and vice versa. The power-moderation paradox (RQ6) can therefore be read as a structural property of the field, while the exploratory nature of the cluster data does not warrant a confirmatory claim.

The 2026 research update adds four directly relevant boundary conditions. Table~\ref{tab:research_update_ledger} does not introduce new ratings or replace the reported empirical outputs; it makes explicit how the new literature should be read against the existing SWR surfaces and where the claim boundary lies.

\begin{table}[htbp]
  \centering
  \caption{2026 research-update ledger for Paper A.}
  \label{tab:research_update_ledger}
  \scriptsize
  \begin{tabularx}{\textwidth}{@{} L{3.0cm} L{2.8cm} L{3.4cm} Y @{}}
    \toprule
    \textbf{2026 source signal} & \textbf{SWR anchor} & \textbf{Presentation gain} & \textbf{Claim boundary} \\
    \midrule
    Techno-supremacy doctrine in AI executive-elite discourse \citep{PerezUrbina2026TechnoSupremacy} & D01/D05/D08/D10; Architect--Innovator contrast & External CDA crosswalk for messianism, inevitability, and risk-as-tech-solution patterns & Supports discourse alignment only; it does not prove that all subgroups share one doctrine. \\[3pt]
    Big-AI regulatory-capture taxonomy \citep{BirhaneEtAl2026RegulatoryCapture} & D07/D11; RQ6; say-do dimensions & Turns the power--equality axis into auditable capture mechanisms for a future v9 ledger & Mechanism candidate, not an actor-intent or causal claim. \\[3pt]
    Infrastructure-sovereignty constraint \citep{Cruzes2026InfrastructureSovereignty} & D07 material power; D11 access; power-moderation paradox & Extends the resource-control reading from discourse and policy to compute, network, energy, and telemetry choke points & Contextual layer only; this dataset does not measure infrastructure ownership directly. \\[3pt]
    GenAI-persona evaluation review \citep{AminEtAl2026GenAIPersonas} & Methodology; limitations; validation overview & Sharpens the distinction between SWR reconstruction and GenAI persona or survey substitution & Supports a method caveat; it is not an external validation of the SWR ratings. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
The primary achievement of the SWR lies in connecting qualitative depth with quantitative rigor at the group level. The study identifies one internally reliable instrument -- Claude Opus~4.6, assessed through Intra-Model Inter-Instance Rating Reliability (IMIIRR, ICC $= 0.902$) -- and applies it consistently across all synthesis units. This IMIIRR coefficient exceeds typical human inter-rater reliability in qualitative coding (ICC $= 0.6$--$0.8$; \citealt{Hallgren2012}), and the instrument offers a distinctive advantage over human coders: fully documented prompts replace implicit coding rules, enabling exact replication. The congruence analysis (RQ3) functions as an internal validity check; the stability of the collective profile across 15~group syntheses and 4~types strengthens convergent validity. The inter-group comparisons provide their own form of triangulation: when different group syntheses -- produced independently from different data subsets -- yield consistent structural patterns (e.g., the D07/D11 axis discriminating across all comparisons), this is evidence for robustness beyond what any single synthesis could provide. Replication with different frontier models constitutes an independent research task -- analogous to replication of a survey with different interviewer teams -- and is explicitly identified as a desideratum for future work rather than a prerequisite for the present study's internal coherence. The limits of the method -- coherence bias, contamination risk, focus control limits, prompt sensitivity -- are systematically analyzed in the limitations section below (Section~\ref{sec:limitationen}).
...
The systematic group-level say-do gap has a policy implication: voluntary commitments -- currently the preferred form of governance \citep[cf.][]{Bengio2024} -- appear insufficient when entire institutional groups (not just isolated individuals) exhibit consistent discrepancies between public rhetoric and observable action. The findings suggest that binding structural reforms may be more effective than further rounds of voluntary commitments.
...
\begin{table}[htbp]
  ...
  \scriptsize
  \begin{tabular}{@{} L{2.9cm} L{1.8cm} L{3.7cm} L{4.0cm} L{0.9cm} @{}}
    \toprule
    ...
    Influence gradient & Post hoc & Strata comparison (3 rank tiers) & Gradient indicated; D09/D10 inverse & QA\,(k) \\[3pt]
...
\emph{Critique of synthetic personas.} The growing literature on LLM-generated synthetic personas counsels caution, but the critique must be carefully scoped. \citet{Santurkar2023} shows that LLMs systematically miss the opinions of specific demographic groups and exhibit ``WEIRD'' biases when used as \emph{opinion generators} -- i.e., when prompted to simulate what a demographic group might think. Similarly, \citet{Argyle2023} find that synthetic agents capture key themes but fail at the authentic depth of real discourse when \emph{generating} qualitative data. A systematic review of 63 peer-reviewed persona experiments (2023--2025) confirms this pattern: most studies underspecify the target population, only 35\,\% address representativeness, and ecological validity is consistently poor \citep{Batzner2025WhosePersonae}. The 81-study scoping review by \citet{AminEtAl2026GenAIPersonas} adds that 45\,\% reported no evaluation, 86\,\% used a single GPT model, and some closed-loop designs used GenAI for both persona creation and evaluation. These critiques do not map one-to-one onto SWR: the present study does not generate respondent opinions but \emph{condenses} 3,132 documented statements and actions into group-level ideal types. That design distinction limits direct transfer of the critique, but does not make it irrelevant. Because synthesis and rating rely on one model family, the internal controls establish instrument-conditional stability rather than external validity; cross-model replication and independent human or source-based validation remain desirable. Alignment between the target population and WEIRD priors likewise does not neutralize bias, because it may amplify publicly dominant narratives and suppress idiosyncratic or contradictory positions. Accordingly, the synthetic personas are not treated as respondents or substitutes for human participants, and the results are interpreted as ideal-typical reconstructions of documented group evidence.
...
\smallskip
\noindent{\large\bfseries Disclosure Statement: Use of Artificial Intelligence\par}

\noindent\textbf{AI Disclosure Level~5 (Extensive).}
In preparing this study, two AI models from Anthropic were used extensively as non-authorial research instruments and editorial tools. They are not listed or treated as authors, co-authors, acknowledgement recipients, ground-truth sources, or independent validation authorities.
...
\emph{Claude Opus~4.6} (model ID: \texttt{claude-opus-4-6}; accessed via Claude Code CLI, February--March 2026; temperature~$= 0$; 1M context window) served as the qualitative analysis engine: group-level syntheses, comparative analyses, statistical processing (PCA, HDBSCAN, Kruskal-Wallis, Cliff's $\delta$), all four post-hoc validation experiments (run-convergence, expected-discrepancy control, aggregation-synthesis comparison, cross-modal prediction), visualization of results, and non-authorial support in structuring and drafting manuscript passages.

No other LLM was used in the original analysis pipeline. A later English editorial style check in May~2026 used OpenAI Codex/GPT for language polishing and claim-calibration only; no new data, sources, analyses, or substantive claims were added in that pass. Research design, research questions, data collection strategy, methodological decisions, interpretation of results, and all substantive conclusions originate from the author, who retains full responsibility for the manuscript. AI outputs were checked by the author and were not treated as external validation or as a substitute for human scholarly judgement. All prompts and model parameters are documented in the repository; a prompt archive is provided in Appendix~\ref{app:prompt}.
...
\smallskip
\noindent{\large\bfseries Data Availability Statement\par}

\noindent All data collection scripts, qualitative coding scripts, analysis code, and generated figures are publicly available at: \url{https://github.com/research-line/ai-elite-swr}. The repository contains 56 collection scripts and 38 insertion scripts in \texttt{\_data\_A/}, coding and analysis scripts, the $100\times12$ rating matrix, HDBSCAN/PCA outputs, prompt templates, dimensional definitions, synthesis units, quality reports, and generated figures. These materials provide an auditable reproduction trail for the implemented pipeline; exact re-execution requires rebuilding the SQLite database from the current scripts and may depend on the availability of the online sources at the time of collection.
\endgroup
...
\bibitem[Amin et~al.(2026)]{AminEtAl2026GenAIPersonas}
  Amin, D., Salminen, J., Ahmed, F., Tervola, S.~M.~H., Sethi, S.~\& Jansen, B.~J. (2026).
  Creating and Evaluating Personas Using Generative AI: A Scoping Review of 81 Articles.
  In \textit{Proceedings of the 2026 CHI Conference on Human Factors in Computing Systems (CHI '26)}, Article~421, 23~pages.
  \url{https://doi.org/10.1145/3772318.3790608}.
...
\bibitem[Birhane et~al.(2026)]{BirhaneEtAl2026RegulatoryCapture}
  Birhane, A., Angius, R., Agnew, W., Pandit, H.~J., Mitra, B., Dobbe, R.~\& Talat, Z. (2026).
  Big AI's regulatory capture: Mapping industry interference and government complicity.
  In: \textit{Proceedings of the 2026 ACM Conference on Fairness, Accountability, and Transparency (FAccT '26)}, pp.~2586--2607, Article~111.
  \url{https://doi.org/10.1145/3805689.3806740}.
...
\bibitem[Cruzes(2026)]{Cruzes2026InfrastructureSovereignty}
  Cruzes, S. (2026).
  AI infrastructure sovereignty.
  \textit{arXiv preprint arXiv:2602.10900}.
  \url{https://arxiv.org/abs/2602.10900}.
...
\bibitem[Ge et~al.(2024)]{Ge2024}
  Ge, T., Chan, X., Wang, X., Yu, D., Mi, H.~\& Yu, D. (2024).
...
\bibitem[Hayes(2025)]{Hayes2025}
  ...
  \textit{International Journal of Qualitative Methods}, \textit{24}, Article 16094069251322346.
  \url{https://doi.org/10.1177/16094069251322346}.
...
\bibitem[Jara-Ettinger(2019)]{JaraEttinger2019}
  ...
  \url{https://doi.org/10.1016/j.cobeha.2019.04.010}.
...
\bibitem[Nicmanis \& Spurrier(2025)]{NicmanisSpurrier2025}
  ...
  Getting Started with Artificial Intelligence Assisted Qualitative Analysis: An Introductory Guide to Qualitative Research Approaches with Exploratory Examples from Reflexive Content Analysis.
  \textit{International Journal of Qualitative Methods}, \textit{24}, Article 16094069251354863.
  \url{https://doi.org/10.1177/16094069251354863}.
...
\bibitem[Tai et~al.(2024)]{Tai2024}
  ...
  \textit{International Journal of Qualitative Methods}, \textit{23}, Article 16094069241231168.
...
\bibitem[Floridi(2019)]{Floridi2019}
  ...
  \url{https://doi.org/10.1038/s42256-019-0055-y}.
...
\bibitem[P\'erez-Urbina(2026)]{PerezUrbina2026TechnoSupremacy}
  P\'erez-Urbina, H. (2026).
  Tracing the techno-supremacy doctrine: A critical discourse analysis of the AI executive elite.
  \textit{AI and Ethics}, \textit{6}, 307.
...
\bibitem[Batzner et~al.(2025)]{Batzner2025WhosePersonae}
  ...
  \textit{Proceedings of the AAAI/ACM Conference on AI, Ethics, and Society}, \textit{8}(1), 343--354.
  \url{https://doi.org/10.1609/aies.v8i1.36553}; arXiv:2512.00461.
...
\bibitem[Soysal \& T\"urkmen(2024)]{SoysalTurkmen2024}
  ...
  \url{https://doi.org/10.59455/qietp.19}.
...
\begin{tabular}{@{} L{1.1cm} L{3.8cm} L{4.8cm} L{4.8cm} @{}}
  \toprule
...
\begin{tabular}{@{} L{1.0cm} L{2.8cm} L{10.8cm} @{}}
  \toprule
...
{\small
\begin{tabular}{@{} r L{3.2cm} L{7.4cm} L{2.0cm} @{}}
  \toprule
