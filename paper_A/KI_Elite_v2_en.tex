...
(e)~\textbf{Anonymization robustness test:} To assess whether the anonymization approach affects the results, the same synthesis unit (risk warners, 139~statements + 92~actions) was processed with two different blinding strategies: (A)~placeholder tokens ([PERSON], [COMPANY]) and (B)~plausible fictional names. The resulting D01--D12 profiles were nearly identical ($r = 0.987$, MAE $= 0.33$, max $|\Delta| = 1$ on a 10-point scale), confirming that the anonymization method does not materially affect group-level ratings. This is expected: the collective worldview of a sociological group is not a retrievable object in the LLM's training data, regardless of naming conventions.
...
\noindent The complete prompt library including synthesis prompts (Op1), rating prompts (Op2), cross-modal prompts (Op4a/Op4b), and dialogue prompts (Op6) is archived in the repository (\texttt{prompts/en/}). For a general discussion of the prompt design rationale, see \citet{Geiger2026SWR}.
