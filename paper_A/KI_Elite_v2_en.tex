...
SWR defines worldview analysis as an inverse problem: from observable statements and actions of a \emph{group}, it infers the collective belief system that would most coherently generate these outputs. The solution consists of constructing a \emph{fictive synthetic person} -- a distillate that represents the group's worldview in pure form, freed from individual biographical noise and PR strategies. In the present study, the primary units of analysis are sociologically defined groups (CEOs, academics, investors, founders, women, men, accelerationists, risk warners, etc.); the 100~individuals serve as data providers whose statements and actions are aggregated at the group level. The theoretical anchoring draws on four traditions: Weber's ideal type, Booth's \emph{implied author}, inverse reinforcement learning, and projective tests. The operative procedure comprises five steps: (1)~compile group-level data corpus, (2)~LLM-assisted synthesis, (3)~structured interview, (4)~meta-reflection, (5)~supplementary prompt variants. For the detailed presentation including the focus control infrastructure, the rationale of the fiction approach, and the validation framework, see the companion paper \citep{Geiger2026SWR}.

\subsubsection{Data Aggregation Procedure}

For each synthesis unit, the LLM receives the complete data corpus of the corresponding group as a single input prompt. For group-level syntheses (e.g., ``all CEOs''), the statements and actions of all group members are concatenated into a single dossier, ordered chronologically. The LLM then constructs a fictional synthetic person whose worldview best explains the totality of the presented group data. For the 100 individual-level profiles used in the algorithmic validation (PCA, HDBSCAN), each actor's data was processed separately. The group-level syntheses and the individual-level profiles thus represent two complementary analytical layers: the former constitutes the study's substantive findings, the latter provides the data matrix for statistical validation.


\subsection{Operationalizations}
\label{sec:methodik:ops}

The three dependent variables (DV1: self-image, DV2: worldview, DV3: view of humanity) are made accessible through six operationalizations defined in the SWR framework, of which four were applied in this study (Op1, Op2, Op3, Op6):
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l l X l}
    ...
    \bottomrule
    \multicolumn{4}{l}{\footnotesize $^*$Op4 was not conducted in this study. Op5 was implemented as PCA + HDBSCAN}\\
...
\subsubsection{Algorithmic Validation of the Cluster Structure}
...
\emph{Aggregation problem.} Collective syntheses level individual variation. The algorithmic validation (PCA, HDBSCAN; Section~\ref{sec:ergebnisse:f4}) at the individual level provides a partial corrective by confirming that the rating matrix exhibits structural variation.
...
In preparing this study, two AI models from Anthropic were used extensively. \emph{Claude Sonnet~4.5} served as the primary analysis engine: all 100~individual-level dimensional ratings (D01--D12), all group-level synthesis ratings, and all data collection scripts were generated by 9~parallel Sonnet agents. \emph{Claude Opus~4.6} served as the qualitative analysis engine: pilot syntheses, comparative analyses, statistical processing (PCA, HDBSCAN, Kruskal-Wallis, Cliff's $\delta$), visualization of results, and support in structuring and drafting the manuscript. No other LLM was used. Research design, research questions, data collection strategy, methodological decisions, interpretation of results, and all substantive conclusions originate from the author. All prompts, API parameters, and model versions are documented in the repository.
...
Additionally: 9 comparison runs (Op6). Cross-modal prediction runs (Op4) were not conducted in this study and remain as a validation desideratum.
