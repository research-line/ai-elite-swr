...
\begin{abstract}
\noindent
Die vorliegende \emph{explorative Studie} rekonstruiert erstmals systematisch das kollektive Weltbild der 100 einflussreichsten Pers\"onlichkeiten im Bereich der k\"unstlichen Intelligenz (KI-Elite) f\"ur den Zeitraum 2010 bis 2026. Methodisch entwickelt und erprobt sie die synthetische Weltbild-Rekonstruktion (SWR) -- ein LLM-gest\"utztes Verfahren, das aus 3.132 Datenpunkten (1.738 \"offentliche Aussagen, 1.394 beobachtbare Handlungen) mittels 51--56 Syntheseeinheiten, 12 Weltbild-Dimensionen und 6 komplementaeren Operationalisierungen koh\"arente \"Uberzeugungssysteme extrahiert (vgl.\ das Companion Paper \citealt{Geiger2026SWR} f\"ur die methodischen Grundlagen). Sechs Forschungsfragen strukturieren die Analyse: F1~(Weltbild) zeigt einen \glqq technologischen Messianismus mit Ambivalenzstruktur\grqq{} (Avg~7,1 Kollektiv, 7,7 Top~10). F2~(Wandel) dokumentiert den \"Ubergang von utopischem Techno-Optimismus zu tragischem Beschleunigungszwang; Transhumanismus f\"allt deutlich ($-2{,}4$/Dekade, $R^2 = 0{,}63$). F3~(Kongruenz) deckt eine systematische Sagen-Handeln-Kluft auf; gr\"o\ss{}te Diskrepanz beim Egalitarismus ($\Delta = -3$). F4~(Cluster) identifiziert vier Weltbild-Typen: Architekt, H\"uter, Innovator, Befreier. F5~(Differenz) benennt die st\"arksten Praediktoren: Haltung ($\Delta$~1,7), Rolle ($\Delta$~1,5), Gender ($\Delta$~1,2). F6~(Macht) offenbart ein Macht-Gem\"a\ss{}igtheits-Paradox: der m\"achtigste Typ ist der extremste, der gem\"a\ss{}igste der machtloseste.
...
Die methodischen Grundlagen der synthetischen Weltbild-Rekonstruktion (SWR) -- inverses Problem, fiktive synthetische Person, Prompt-Protokoll, Blindungsinfrastruktur und Validierungsrahmen -- werden im Companion Paper \citep{Geiger2026SWR} ausf\"uhrlich dargelegt. Der vorliegende Abschnitt beschr\"ankt sich auf die anwendungsspezifischen Aspekte: Datenerhebung, Operationalisierungen, Forschungsdesign und Qualit\"atssicherung dieser Studie.
...
\begin{itemize}[nosep]
  ...
  \item UV5 \emph{Kongruenzstatus:} Konsistent vs.\ widerspruchlich
...
Die methodische Qualit\"at wird durch sechs Massnahmen gesichert: (1)~Mindestgr\"o\ss{}e von 15 Datenpunkten pro SE, (2)~Inter-LLM-Reliabilit\"at (zwei Modelle, ICC-Berechnung), (3)~Sensitivit\"atsanalyse (mindestens drei Runs pro SE), (4)~konsequente Blindung (Entfernung aller Identifikatoren), (5)~Reproduzierbarkeitsprotokoll (Temperatur~0, vollst\"andige Prompt-Dokumentation) und (6)~ein systematischer Halluzinations-Check. F\"ur eine ausf\"uhrliche Diskussion der Validierungsstrategien und ihrer Grenzen verweisen wir auf \citet{Geiger2026SWR}.

\subsubsection{Halluzinations-Check und Positionsvalidierung}

Da LLM-gest\"utzte Rekonstruktionen grunds\"atzlich anf\"allig f\"ur Halluzinationen sind --- d.\,h.\ das Modell k\"onnte Positionen zuschreiben, die eine Person nie vertreten hat ---, wird ein systematischer Validierungsplan implementiert:

\begin{description}[style=nextline, leftmargin=1.5cm]
  \item[\textbf{Stratified Spot-Check (10\%):}] Eine geschichtete Zufallsstichprobe von 10 der 100 Personen (je 2--3 pro Cluster-Typ) wird f\"ur eine vollst\"andige manuelle Gegen\-pr\"ufung ausgew\"ahlt. F\"ur jede dieser Personen werden alle 12 Dimensionsratings mit dokumentierten \"offentlichen Aussagen abgeglichen. Abweichungen $> 2$ Skalenpunkte zwischen rekonstruiertem Rating und dokumentierter Position werden als \glqq materielle Diskrepanz\grqq{} klassifiziert und im Supplementary Material berichtet.
  \item[\textbf{Pr\"ufprotokoll:}] Jedes Rating der 10 validierten Personen erh\"alt einen Belegstatus: \emph{best\"atigt} (dokumentierte Aussage/Handlung st\"utzt das Rating), \emph{plausibel} (keine direkte Evidenz, aber konsistent mit dem \"offentlichen Profil) oder \emph{nicht belegbar} (keine Evidenz gefunden, m\"ogliche LLM-Halluzination). Zielwert: $\geq 80$\% best\"atigte oder plausible Ratings.
  \item[\textbf{Red-Flag-Analyse:}] Zus\"atzlich werden alle Extremwerte ($\leq 2$ oder $\geq 9$) im gesamten Datensatz ($100 \times 12 = 1.200$ Ratings) auf Plausibilit\"at gepr\"uft, da Extrempositionen das h\"ochste Halluzinationsrisiko tragen. F\"ur Extremwerte wird jeweils mindestens eine dokumentierte Quellenangabe verlangt.
\end{description}

\noindent Dieser Halluzinations-Check adressiert das zentrale methodische Risiko der SWR und stellt sicher, dass die berichteten Befunde nicht auf fabrizierten Positionen beruhen, sondern auf empirisch verankerbaren Rekonstruktionen.

\subsection{Rolle des LLM im Forschungsprozess}
\label{sec:methodik:llm-rolle}

Die Rolle des Large Language Models (Claude Opus 4.6) im Forschungsprozess muss pr\"azise bestimmt werden. Das LLM fungiert in dieser Studie in einer dreifachen Rolle:

\begin{description}[style=nextline, leftmargin=1.5cm]
  \item[\textbf{(a) Analyseinstrument:}] Das LLM verarbeitet die Datenpunkte (Aussagen und Handlungen) und erzeugt daraus synthetische Weltbild-Rekonstruktionen. In dieser Funktion ist es vergleichbar mit einem qualitativen Kodierungswerkzeug -- es ersetzt den menschlichen Kodierer bei der Mustererkennung und Synthese.
  \item[\textbf{(b) Wissenstraeger:}] Da das LLM auf umfangreichen Textkorpora trainiert wurde, die Aussagen und Handlungen der untersuchten Personen enthalten, bringt es eigenes Kontextwissen ein. Dies ist gleichzeitig St\"arke (Kontextualisierung) und Risiko (Kontamination). Die Blindungsinfrastruktur adressiert dieses Risiko, kann es aber nicht vollst\"andig eliminieren.
  \item[\textbf{(c) Formulierungshilfe:}] Bei der Erstellung des Manuskripts unterst\"utzte das LLM die Strukturierung und Formulierung. Forschungsdesign, Interpretation und Schlussfolgerungen stammen vom menschlichen Autor.
\end{description}

\noindent\textbf{Wichtig:} Das LLM ist \emph{nicht} die Datenquelle. Die Datenquelle sind die 3.132 empirisch erhobenen Datenpunkte (\"offentliche Aussagen und beobachtbare Handlungen). Das LLM ist das Instrument, das diese Daten zu koh\"arenten Weltbildern verdichtet.

\subsection{Validierungsstrategie}
\label{sec:methodik:validierung}

Die Validierung der SWR-Ergebnisse erfolgt auf drei Ebenen:

\begin{description}[style=nextline, leftmargin=1.5cm]
  \item[\textbf{Interne Konsistenz:}] Die Kongruenzanalyse (F3) vergleicht aus Aussagen und aus Handlungen rekonstruierte Weltbilder derselben Person. Systematische Diskrepanzen deuten nicht auf Methodenfehler, sondern auf reale Sagen-Handeln-Kluefte hin -- und stellen damit einen inhaltlichen Befund dar.
  \item[\textbf{Konvergenzvalidit\"at:}] Die Stabilit\"at des Kollektivprofils \"uber 15 Gruppensynthesen und 4 Cluster-Typen st\"utzt die Konvergenzvalidit\"at. Zus\"atzlich werden die Befunde mit den qualitativen Diagnosen der bestehenden Literatur \citep{BarbCam1996, Morozov2013, Daub2020, TorresGebru2024} verglichen.
  \item[\textbf{Geplante externe Validierung --- Detaillierter Studienplan:}] Um die Validit\"at der SWR-Rekonstruktionen empirisch zu testen, wird folgende Validierungsstudie f\"ur eine Stichprobe von $N = 10$ Personen aus der Gesamtstichprobe vorgeschlagen:

    \textbf{Stichprobenauswahl:} 10 Personen, stratifiziert nach Clustern (3 Architekten, 3 H\"uter, 2 Innovatoren, 2 Befreier), ausgew\"ahlt nach Kriterien: (a)~substantielle sekund\"are Literatur verf\"ugbar (Biografien, kritische Analysen, Interview-Sammlungen), (b)~\"offentlich dokumentierte Handlungsmuster (Abstimmungsverhalten, Spendenmuster, Organisationszugeh\"orighkeiten), (c)~hohe dimensionale Differenzierung (keine zentristischen Profile).

    \textbf{Vergleichsquellen:} F\"ur jede Person werden drei unabh\"angige Validierungsquellen herangezogen: (1)~Publizierte Biografien oder kritische Portr\"ats, (2)~strukturierte Interviews (z.\,B.\ Lex Fridman Podcast, The 80,000 Hours Podcast), (3)~\"offentlich dokumentiertes Handeln (z.\,B.\ politische Spenden via FEC-Datenbank, Open-Source-Beiträge, Organisationsmitgliedschaften).

    \textbf{Validierungsprozedur:} Drei unabh\"angige Kodierer (Doktoranden der Sozialwissenschaften, mit KI-Governance-Expertise) erhalten f\"ur jede Person: (a)~die SWR-generierte dimensionale Bewertung (12 Dimensionen, Skala 1--10), (b)~die drei Vergleichsquellen, (c)~eine Kodieranleitung f\"ur jede Dimension. Kodierer bewerten jede Dimension anhand der Vergleichsquellen (Skala 1--10). Interrater-Reliabilit\"at wird via ICC gemessen (Akzeptanzkriterium: ICC $\geq 0{,}70$).

    \textbf{Konvergenzanalyse:} Pearson-Korrelation zwischen SWR-Scores und Experten-Ratings pro Dimension. Hypothese: $r \geq 0{,}60$ f\"ur Kerndimensionen (Individualismus, Techno-Optimismus, Regulierungspräferenz). Systematische Divergenzen (z.\,B.\ SWR konsequent h\"oher) identifizieren methodische Verzerrungen (z.\,B.\ Koh\"arenzbias).

    \textbf{Qualitative Diskrepanzanalyse:} F\"ur die 3 Fälle mit h\"ochster Divergenz ($r < 0{,}40$) erfolgt eine qualitative Fehleranalyse: Welche Aspekte wurden vom LLM \"uber-/unterbetont? Welche Quellenarten (Aussagen vs.\ Handlungen) produzieren systematische Verzerrungen?

    \textbf{Falsifikationskriterium:} Falls die mittlere Korrelation \"uber alle Dimensionen $\bar{r} < 0{,}50$ oder mehr als 4/10 F\"alle $r < 0{,}40$ zeigen, ist die Methode nicht valide genug f\"ur wissenschaftliche Anwendung --- dies erfordert substanzielle Revision des Prompt-Protokolls oder R\"uckzug auf rein heuristische Verwendung.

    Diese Validierung wurde im Rahmen der vorliegenden explorativen Studie noch nicht durchgef\"uhrt und ist Gegenstand zuk\"unftiger Arbeiten (Geiger, in Vorbereitung).
\end{description}

\noindent\textbf{Epistemischer Status der Befunde:} Die vorliegende Studie versteht sich als \emph{explorative Methodendemonstration}. Die quantitativen Befunde (Ratings, Trends, Korrelationen) beschreiben Muster in den LLM-generierten Rekonstruktionen und erheben keinen Anspruch auf die Pr\"azision standardisierter psychometrischer Instrumente. Sie generieren Hypothesen, die durch unabh\"angige Methoden (Surveys, Expertenratings, Inhaltsanalysen) zu pr\"ufen w\"aren.
...
Den st\"arksten R\"uckgang verzeichnet der Posthumanismus: von 10 im Jahr 2010 auf 6 im Jahr 2025. Die hohe Korrelation mit D12 ($r = +0{,}85$) weist auf ein zusammenh\"angendes Ph\"anomen: Transhumanismus und Zukunftszuversicht erodieren synchron. Der einzige deutlich steigende Wert betrifft die Menschenfreundlichkeit (D09), wobei die negative Korrelation mit D10 ($r = -0{,}58$) eine kompensatorische Verkn\"upfung nahelegt. Der Gesamtdurchschnitt bleibt nahezu konstant (Fr\"uhphase 7,5, Sp\"atphase 7,3), was eine tektonische Verschiebung unter der Oberfl\"ache verbirgt.
...
\medskip
\noindent\textbf{Kernbefund F2:} Das Weltbild hat sich strukturell transformiert -- von einer monochromen Utopie zu einer bimodalen Wette. Drei Erosionstrends (D10, D12, D02) bilden einen \glqq Utopie-Komplex\grqq, dessen Zerfall durch steigende Dringlichkeit arithmetisch kompensiert wird.
...
\medskip
\noindent\textbf{Kernbefund F3:} Die Inkongruenz ist systematisch, gerichtet und funktional. Die KI-Elite meint, was sie \"uber ihre eigene Bedeutung sagt -- aber nicht, was sie \"uber Gleichheit, Verantwortung und Menschlichkeit sagt.
...
Die KI-Elite vertritt einen \emph{technologischen Messianismus mit struktureller Ambivalenz}. Die sechs Forschungsfragen f\"ugen sich zu einem koh\"arenten Bild: F1~zeigt die \glqq Doppelsenke\grqq{} bei D09 und D11; F2~offenbart die Erosion des Utopie-Komplexes; F3~liefert den brisantesten Einzelbefund (systematische Sagen-Handeln-Kluft); F4~identifiziert vier Typen mit antagonistischen Positionen; F5 und F6 vervollst\"andigen das Bild durch das Macht-Gem\"a\ss{}igtheits-Paradox.

\"Uberraschend ist erstens das Ausma\ss{} der Sagen-Handeln-Kluft (nicht vereinzelte Abweichung, sondern funktionale Struktur); zweitens der R\"uckzug des Transhumanismus (widerlegt die g\"angige Annahme zunehmender posthumanistischer Orientierung); drittens die Existenz des Befreier-Typs (Opposition gegen Machtkonzentration auch von libert\"arer Seite).
...
Die von \citet{BarbCam1996} diagnostizierte Californian Ideology findet in der Existenz des Befreier-Typs ihre zeitgen\"ossische Entsprechung -- empirisch nachweisbar fast drei\ss{}ig Jahre sp\"ater.

\citeauthor{Coeckelbergh2020}s Annahme einer zunehmenden Enhancement-Orientierung wird partiell \emph{widerlegt}: Der R\"uckgang des Transhumanismus (D10 von 10 auf 6) zeigt, dass die KI-Elite sich weniger f\"ur die Verbesserung des Menschen interessiert als f\"ur dessen \"Uberbietung durch Maschinen.
...
Die systematische Sagen-Handeln-Kluft hat eine Policy-Implikation: Freiwillige Selbstverpflichtungen -- die derzeit bevorzugte Governance-Form -- sind unter diesen Bedingungen unzureichend. Verbindliche Strukturreformen w\"aren wirksamer als weitere Runden freiwilliger Commitments.
...
\emph{SWR-Methode in Entwicklung.} Die verwendete Synthetische Weltbild-Rekonstruktion (SWR) ist eine neuartige Methode, die in \citet{Geiger2026SWR} theoretisch entwickelt wurde, aber noch keine umfassende empirische Validierung an unabh\"angigen Datens\"atzen erfahren hat. Die vorliegende Studie ist eine erste Anwendung, die gleichzeitig als Proof-of-Concept f\"ur die Methode selbst dient. Zuk\"unftige Forschung sollte die SWR an etablierten Benchmarks gegen\"uber klassischen qualitativen Verfahren evaluieren.

\emph{Zirkularit\"atsproblem und methodische Kontrollmaßnahmen.} Ein grunds\"atzliches methodisches Problem besteht darin, dass LLMs Weltbilder aus Texten rekonstruieren, die m\"oglicherweise Teil ihrer Trainingsdaten waren. Dies ist besonders problematisch f\"ur diese Studie, da viele der analysierten Personen (z.\,B.\ Yann LeCun, Demis Hassabis, Sam Altman) direkt an der Entwicklung oder dem Training der verwendeten LLMs beteiligt waren. Dies erzeugt ein fundamentales Zirkularit\"atsrisiko: Das LLM k\"onnte vorgefertigte Interpretationen reproduzieren statt genuine Rekonstruktionsarbeit zu leisten.

\paragraph{Empirische Kontrollmaßnahmen.} Um die Zirkularit\"at quantifizierbar zu machen, haben wir f\"unf methodische Kontrollen konzipiert, die in zuk\"unftigen Arbeiten systematisch durchgef\"uhrt werden sollen:

\begin{enumerate}[leftmargin=*, noitemsep]
    \item \textbf{Konsequente Anonymisierung:} Alle Identifikatoren (Namen, Organisationen, spezifische Projekte) werden aus dem Input-Korpus entfernt, bevor das LLM die Rekonstruktion durchführt. Damit wird direkte Kontamination verhindert.

    \item \textbf{Blindierungs-Kontrollexperiment ($N = 20$ Personen):} F\"ur eine Teilstichprobe von 20 Personen werden sowohl blindierte (Namen entfernt) als auch nicht-blindierte Rekonstruktionen durchgeführt. Die dimensionalen Scores werden verglichen. \textit{Erwartung:} Bei echter Rekonstruktion sollten blindierte und nicht-blindierte Versionen hochkorreliert sein ($r \geq 0{,}80$). Falls $r < 0{,}60$, deutet dies auf substantielle Kontamination hin --- die nicht-blindierte Version reproduziert vorgefertigte Frames statt genuine Analyse. \textit{Falsifikationskriterium:} Falls $r < 0{,}50$ f\"ur $> 30$\,\% der Stichprobe, ist die Methode kontaminiert und nicht valide.

    \item \textbf{Inter-LLM-Divergenztest:} Rekonstruktionen werden mit mindestens drei verschiedenen LLMs durchgeführt (Claude Opus~4.6, GPT-4o, Gemini~1.5 Pro). \textit{Hypothese bei Zirkularit\"at:} Falls ein LLM vorgefertigte Interpretationen hat (z.\,B.\ Claude reproduziert Anthropics Sichtweise auf Dario Amodei), sollte dieses LLM \textit{weniger} mit den anderen LLMs korrelieren. \textit{Test:} F\"ur jeden der 100 F\"alle wird die paarweise LLM-Korrelation berechnet. Systematisch niedrige Korrelationen ($r < 0{,}60$) f\"ur spezifische LLM-Person-Kombinationen identifizieren Kontaminationsf\"alle.

    \item \textbf{Temporal-Shift-Analyse:} F\"ur 15 Personen mit ausreichend Daten werden separate Rekonstruktionen für 2010--2015 vs.\ 2020--2025 erstellt. \textit{Zirkularit\"atshypothese:} Falls das LLM nur vorgefertigte Frames reproduziert, sollten die Rekonstruktionen temporal stabil sein (keine Ver\"anderung). Falls genuine Rekonstruktion vorliegt, sollten sich die Weltbilder \"uber Zeit ver\"andert haben (z.\,B.\ Elon Musk 2010 vs.\ 2024). \textit{Erwartung:} Mindestens 8/15 Personen zeigen substanzielle Verschiebung ($\Delta \geq 1{,}5$ Punkte auf mindestens 3 Dimensionen).

    \item \textbf{Out-of-Distribution-Test:} Drei Personen aus der Stichprobe, die mit sehr hoher Wahrscheinlichkeit \textit{nicht} in den Trainingsdaten moderner LLMs sind (z.\,B.\ j\"ungere, nicht-anglophone KI-Forscher aus Indien/S\"udamerika), werden als Kontrollgruppe analysiert. Falls die Rekonstruktionen f\"ur diese Personen substantiell schw\"acher sind (niedrigere Divergenzmetrik $D$, h\"ohere Inter-LLM-Varianz), deutet dies darauf hin, dass die Methode f\"ur bekannte Personen von Trainingswissen profitiert.
\end{enumerate}

\paragraph{Epistemischer Status.} Diese Kontrollmaßnahmen k\"onnen die Zirkularit\"at nicht eliminieren, aber quantifizierbar machen. \textbf{Die oben beschriebenen f\"unf Kontrollen sind als systematischer Validierungsrahmen konzipiert, wurden aber in der vorliegenden Studie noch nicht vollst\"andig durchgef\"uhrt.} Die vorliegende Arbeit dient als initiale Anwendung der SWR-Methode; die umfassende empirische Validierung einschlie\ss{}lich aller Zirkularit\"atskontrollen ist Gegenstand einer geplanten Folgestudie. Die Studie erhebt daher keinen Anspruch auf kontaminationsfreie Rekonstruktion, sondern pr\"asentiert explorative Befunde, deren Robustheit durch die beschriebenen Kontrollen zuk\"unftig getestet werden muss. Die fundamentale Frage, inwieweit LLM-basierte Rekonstruktionen epistemisch valide sind, wenn das LLM selbst bereits \"uber interpretative Frames verf\"ugt, wird ausf\"uhrlich in \citet{Geiger2026SWR} diskutiert.
...
\emph{Reproduzierbarkeit.} LLM-Outputs sind auch bei Temperatur~0 nicht vollst\"andig deterministisch. F\"ur die vorliegende Studie wurden verwendet: \textbf{Claude Opus 4.6} (Anthropic, Stand: Januar 2025), \textbf{Temperatur: 0.0}, \textbf{System-Prompt:} siehe Anhang A, \textbf{Runs pro Syntheseeinheit: 3}. Mindestens drei Runs pro SE sichern die Robustheit. Die vollst\"andigen Prompts und Modellspezifikationen sind in den Supplementary Materials dokumentiert.

\emph{Validierungsstatus der Zirkularit\"atskontrollen.} Die oben beschriebenen f\"unf Kontrollen (Anonymisierung, Blindierungs-Experiment, Inter-LLM-Divergenztest, Temporal-Shift-Analyse, Out-of-Distribution-Test) sind als systematischer Validierungsrahmen konzipiert, wurden aber in der vorliegenden Studie noch nicht umgesetzt. \textbf{Geplante Kontrollen:} Alle f\"unf Kontrollen sollen in einer Folgearbeit durchgef\"uhrt werden, um die Robustheit der SWR-Methode gegen\"uber dem Zirkularit\"atsrisiko empirisch zu pr\"ufen. \textbf{Durchgef\"uhrte Kontrollen:} Die vorliegende Studie verwendet ausschlie\ss{}lich Claude Opus~4.6 mit konsistenten Prompts (Temperatur~0.0, drei Runs pro SE), aber ohne systematische Inter-LLM-Triangulation oder experimentelle Validierung. Die Befunde sind daher als explorative Pilotergebnisse zu verstehen, deren methodische Validit\"at durch die geplanten Kontrollen noch zu demonstrieren ist.


\subsection{Konzeptuelle Limitationen}

\emph{Aggregationsproblem.} Kollektive Synthesen glätten individuelle Variation. Die Netzwerk-Analyse (Op5) auf Individualebene bildet ein Korrektiv.
...
\emph{Drittens:} Die \"offentlich kommunizierten \"Uberzeugungen weichen systematisch vom Handeln ab -- besonders beim Versprechen der Demokratisierung ($\Delta\,-3$).
...
\noindent Alle Datenerhebungsskripte, qualitativen Kodierskripte, Analysecode und generierten Abbildungen sind \"offentlich verf\"ugbar unter: \url{https://github.com/lukisch/ai-elite-worldview}. Das Datenbankschema und 85 akteursspezifische Erhebungsskripte erm\"oglichen die vollst\"andige Reproduktion der Datenpipeline. Das Companion Paper zur SWR-Methode ist verf\"ugbar unter: \url{https://doi.org/10.5281/zenodo.18736720}.
...
Die Stichprobe wurde durch ein dreistufiges Verfahren erstellt:

\textbf{Stufe~1: Kandidatenidentifikation.} 22 systematische Webrecherchen in den Kategorien \glqq Top AI researchers\grqq{}, \glqq Most influential people in AI\grqq{}, \glqq AI CEOs and founders\grqq{}, \glqq AI policy makers\grqq{}, \glqq AI ethics leaders\grqq{} und \glqq AI critics\grqq{} ergaben 187 Kandidatinnen und Kandidaten. Quellen umfassten Forbes, MIT Technology Review, Time 100 AI, Wired, Nature Machine Intelligence, Stanford HAI und einschl\"agige Medienberichte.

\textbf{Stufe~2: Einfluss-Scoring.} Jede Person wurde auf sechs Dimensionen (Skala 0--10) bewertet: (i)~Marktkapitalisierung/Ressourcenkontrolle (Firmenwert, Forschungsbudget, Datenzugang), (ii)~technologische Schl\"usselpositionen (CEO, CTO, Research Director, Chefarchitekt relevanter Systeme), (iii)~\"offentliche Diskursmacht (Medienpr\"asenz, Social-Media-Reichweite, Keynote-Einladungen), (iv)~akademischer Einfluss (h-Index, Zitationen, Preistr\"ager), (v)~politische Einflussnahme (Regierungsberatung, Lobbying, Anh\"orungen), (vi)~symbolischer Status (kulturelle Ikonizit\"at, Wiedererkennung au\ss{}erhalb der Fachcommunity). Die Dimensionen wurden gleichgewichtet; der Gesamtscore ergibt sich als arithmetisches Mittel. Das Scoring wurde durch den Erstautor durchgef\"uhrt und durch LLM-gest\"utzte Plausibilit\"atspr\"ufung gegengepr\"uft.

\textbf{Stufe~3: Reduktion und Quotierung.} Die 187 Kandidaten wurden nach Gesamtscore absteigend sortiert. Die Reduktion auf 100 erfolgte unter drei Nebenbedingungen: (a)~mindestens 15\% Frauen (erreicht: 18\%), (b)~mindestens 4 Rollengruppen vertreten (Unternehmer, Forscher, Regulierer, Kritiker; erreicht: 6 Rollen), (c)~bei Score-Gleichstand wurde die Person mit gr\"o\ss{}erer demographischer Diversit\"at bevorzugt. Der niedrigste aufgenommene Gesamtscore betrug 4,2/10.

Die finale Stichprobe umfasst 1.738 Aussagen und 1.394 Handlungen (3.132 Datenpunkte). Eine vollst\"andige Liste der 100 Personen mit Scoring-Werten ist im Supplementary Material verf\"ugbar.
...
\begin{longtable}{r l l l}
  ...
  42--43 & KG\_*\_AH & Konsistent, Widerspruchlich & F3c \\
  \bottomrule
\end{longtable}
}
