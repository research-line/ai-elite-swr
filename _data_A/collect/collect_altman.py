...
collect_altman.py
=================
Collects verifiable statements (aussagen) and actions (handlungen) of Sam Altman (person_id=2)
and inserts them into the SQLite database aussagen_top100.db.

SOURCES: All quotes originate from publicly accessible interviews,
blog posts, Congressional testimony, and news articles.
Each statement is accompanied by a verifiable source.
...
# AUSSAGEN (Statements)
# ============================================================================
# Each statement (aussage) contains:
#   - aussage_text: Original wording (English)
#   - aussage_kurz: 1-sentence summary (German)
#   - modus: 'muendlich' (oral) or 'schriftlich' (written)
#   - quellen_typ_id: FK to quellen_typen (source types)
#   - plattform_id: FK to plattformen (platforms)
#   - quell_link: URL
#   - quell_titel: Source title
#   - datum_aussage: YYYY-MM-DD or YYYY
#   - sprache: 'en'
#   - kontext: Context
#   - aussage_uebersetzung_de: German translation
...
def insert_data():
    """Inserts all collected statements and actions into the database."""
    ...
    # Check if person_id=2 exists
    cursor.execute("SELECT name FROM personen WHERE id = ?", (PERSON_ID,))
    person = cursor.fetchone()
    if not person:
        print(f"ERROR: Person with id={PERSON_ID} not found in database.")
        conn.close()
        return
    print(f"Person found: {person[0]} (id={PERSON_ID})")
    ...
    print(f"Existing entries: {existing_a} statements, {existing_h} actions")

    # --- Insert statements (aussagen) ---
    aussagen_count = 0
    skipped_a = 0
    for a in AUSSAGEN:
        # Duplicate check: same text for same person
        cursor.execute(
    ...
    # --- Insert actions (handlungen) ---
    handlungen_count = 0
    skipped_h = 0
    for h in HANDLUNGEN:
        # Duplicate check: same type + date + person
        cursor.execute(
    ...
    # --- Summary ---
    print(f"\n{'='*60}")
    print(f"  RESULT: Sam Altman (person_id={PERSON_ID})")
    print(f"{'='*60}")
    print(f"  New statements inserted:     {aussagen_count}")
    print(f"  Statements skipped:          {skipped_a} (duplicates)")
    print(f"  New actions inserted:        {handlungen_count}")
    print(f"  Actions skipped:             {skipped_h} (duplicates)")
    print(f"  Search log created:          Yes")
    print(f"{'='*60}")
    ...
    print(f"\n  TOTAL in DB: {total_a} statements, {total_h} actions for Sam Altman")
...
if __name__ == "__main__":
    ...
    print("  Verified statements & actions: Sam Altman")
    print("=" * 60)
    print(f"\nDatabase: {DB_PATH}")
