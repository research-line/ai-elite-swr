"""
Phase 7.20: Statistische Zeitreihen-Analyse der Weltbild-Dimensionen D01-D12
...
def linear_trend(years, values):
    """Lineare Regression: Steigung, R², p-Wert"""
...
def detect_breakpoints(values, window=3):
    """Einfache Breakpoint-Detection via gleitendem Mittelwert-Differenz"""
    breakpoints = []
    for i in range(window, len(values) - window):
        ...
        if abs(diff) >= 1.5:
            breakpoints.append({
                ...
                "delta": round(diff, 2),
                "direction": "Anstieg" if diff > 0 else "Abfall"
            })
    return breakpoints

def correlation_matrix():
    """Korrelationsmatrix zwischen allen 12 Dimensionen"""
...
def divergence_analysis():
    """Dimensionen die auseinanderlaufen (erste Haelfte vs. zweite Haelfte)"""
    mid = len(YEARS) // 2
    results = []
...
def variance_ranking():
    """Dimensionen nach Varianz sortiert"""
    results = []
    for name, vals in DIMS.items():
        results.append({
            "dimension": name[:3],
...
def year_of_doubt_analysis():
    """Spezialanalyse: 2019 als 'Jahr des Zweifels'"""
...
def chatgpt_effect():
    """Spezialanalyse: ChatGPT-Effekt (2021 vs. 2023)"""
...
def generate_report():
    """Hauptfunktion: Generiert den vollstaendigen Analysebericht"""
    ...
    print(f"Zeilen: {len(report)}")

    # Auch als JSON fuer Visualisierungen
    json_data = {
        "years": YEARS,
        "n_ges": N_GES,
    ...
    print(f"JSON-Daten geschrieben: {json_path}")
